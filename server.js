...
app.get("/", (req, res) => {
  ...
  let month =
    dateObj.getUTCMonth() + 1 < 10
      ...
      : dateObj.getUTMonth() + 1;
  res.render("index", {
    currentDate: dateObj.getUTCFullYear() + "-" + month + "-" + day
  });
});
...
app.post("/api/exercise/add", async (req, res) => {
  ...
  try {
    ...
    if (userIdLen == 0) {
      // if the user is not in the db, return an error & don't add the exercise
      res.json({
        message:
          "UserId is not valid. Add an Exercise for a User with a valid UserId."
      });
    } else {
      // if the user is in the system: find the username & create a new instance of the exerice
      let userInSystem = User.find({ _id: userId }).then(user => {
